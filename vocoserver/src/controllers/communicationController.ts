...
===================================================== */
export const sendCommunication = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (!message) return next(new ValidationError("Message requis"));
  if (!["sms", "whatsapp"].includes(channel)) {
    return next(new ValidationError("Canal invalide (sms ou whatsapp)"));
  }
  ...
  switch (recipients) {
    ...
    case "stores_by_city": {
      if (!city) return next(new ValidationError("Ville requise"));
...
const stores = await Store.find({ city: new RegExp(escapedCity, "i") }).select("phone storeName ownerName").lean();
    ...
    default:
      return next(new ValidationError("Destinataires invalides"));
  }

  if (phones.length === 0) {
    return next(new ValidationError("Aucun destinataire trouvé"));
  }
...
===================================================== */
export const getCommunicationHistory = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
...
===================================================== */
export const getCommunicationStats = asyncHandler(async (_req: Request, res: Response, next: NextFunction) => {
