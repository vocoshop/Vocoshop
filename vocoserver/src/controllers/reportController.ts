...
export const viewSharedReport = async (req: Request, res: Response) => {
  try {
    ...
    // VocoScore (blockchain) — plus complet, idéal pour les institutions
    // computeScore a besoin du shopId pour trouver le Store, mais utilise storeId pour le reste
    let totalScore = 0;
    try {
      const shopIdForScore = String((store as any)?.shopId || storeId);
      const scoreData = await computeScore(shopIdForScore);
      totalScore = Math.round((scoreData.overallScore || 0) / 10);
    } catch (e) {
      console.error("VocoScore error:", (e as Error).message);
      totalScore = Math.min(100, Math.max(10, Math.round(totalProductsCount * 2 + 10)));
    }
