...
const animateToStep = (target: "phone" | "password") => {
  ...
  // Reauth : session expir�e, on va direct au mot de passe
  useEffect(() => {
    if (reauth) {
      (async () => {
        try {
...
return;
  ...
  const resetPassword = async () => {
    const cleanPhone = buildFullPhone();
    if (!cleanPhone || cleanPhone.length < 8) return Alert.alert("", "Entre un num�ro valide.");
    if (!recoveryCode || recoveryCode.length < 4) return Alert.alert("", "Entre le code de r�cup�ration.");
    ...
    try {
      ...
      Alert.alert("Succ�s", "Mot de passe r�initialis�. Connectez-vous.", [
        { text: "OK", onPress: () => { setShowForgot(false); setNewPassword(""); setRecoveryCode(""); animateToStep("password"); } }
      ]);
    } catch (e: any) {
      Alert.alert("Erreur", e?.response?.data?.error || "�chec.");
...
<ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
        ...
        <Animated.View style={{ opacity: fadeAnim, transform: [{ translateY: slideAnim }] }}>
          ...
          <Text style={styles.title}>
            {reauth ? "Session expir�e" : step === "phone" ? "Bienvenue sur Vocoshop" : "Bienvenue"}
          </Text>

          <Text style={styles.subtitle}>
            {reauth ? "Inactivit� prolong�e. Entrez votre mot de passe." :
             step === "phone" ? "La gestion simple et intelligente de votre activit�." : "Entrez votre code secret 6 chiffres"}
          </Text>
          ...
          {showForgot ? (
            <View>
              ...
              >
                {forgotLoading ? <ActivityIndicator color="#fff" /> : <Text style={styles.btnText}>R�initialiser</Text>}
              </TouchableOpacity>
              <TouchableOpacity style={{ marginTop: 12 }} onPress={() => setShowForgot(false)}>
...
/>
                ...
                <Text style={styles.infoText}>Entrez votre num�ro de t�l�phone pour commencer.</Text>
...
{loading ? <ActivityIndicator color="#fff" /> : <Text style={styles.btnText}>Se connecter</Text>}
        ...
        <TouchableOpacity style={styles.backBtn} onPress={() => { setForgotStep(""); setOtpCode(""); setNewPassword(""); resetForm(); }}>
          <Text style={styles.backBtnText}>Changer de num�ro</Text>
        </TouchableOpacity>

        <TouchableOpacity style={{ marginTop: 20 }} onPress={() => setShowForgot(true)}>
          <Text style={{ color: "#A78BFA", fontSize: 13, fontWeight: "600", textAlign: "center" }}>
            {forgotLoading ? "Envoi du code..." : "Mot de passe oubli� ?"}
          </Text>
        </TouchableOpacity>
